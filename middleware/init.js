...
module.exports = (options) => {
	...
	return async function auth(ctx, next) {
		...
		//后台系统配置
		ctx.sysconfig = await ctx.memorycache("basic-config" + APPID, false, 3600, async (configs) => {
			console.log("开始查询系统配置")
			const data = await db.collection("tian-identity").find({
				$or: [
				  {
					dcloud_appid: false,
					type: "sysconfig"
				...
				data: 1,
				dataType: 1
			  }).sort({key :1}).limit(500);

			let allData = data.map(e => {
				//转换数据
			...
			//公共配置
			let commonConfig = allData.filter(e => !e.dcloud_appid || e.dcloud_appid.length ==
				0);
			//应用特有配置
			let myConfig = allData.filter(e => e.dcloud_appid && e.dcloud_appid.indexOf(
				APPID) != -1);
			//合并uni-id内参数
			let outObj = {}
			if (APPID) {
			...
			commonConfig.map(item => {
				let obj = item.key.split(".").reverse().reduce((total, item) => {
					return {
						[item]: total
					};
				}, item.data);
		...
		ctx.uniID = uniID.createInstance({ // 自行创建uni-id实例，传入context，后续均使用此uniID调用相关接口
			context: ctx.context,
			config: ctx.sysconfig // 完整uni-id配置信息，使用config.json进行配置时无需传此参数
		})
		await next() // 执行后续中间件
	}
}
